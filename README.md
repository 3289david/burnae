...
일반 서버 — VPS류는 일반 서버에 포함)로 분류해 고를 수 있습니다. 유저가 코드 저장소 주소나 설정값을
채워 완전히 새로운 서버 종류("커뮤니티 프리셋")를 만들어 공개하고 포인트를 받을 수도 있습니다. 가짜
데이터/시뮬레이션 없이 실제 Pterodactyl API, 하나은행 Open API 결제, Cloudflare DNS, 디스코드 봇과
연동합니다.
...
- **홍보 포인트 & 포인트 상점**: 친구 추천(`/register?ref=내추천코드`), 디스코드 가입, 블로그/영상/
  ...
  영구히 차지하는 걸 막기 위함
- **커뮤니티 프리셋**: `/dashboard/community-eggs`(대시보드 상단 탭, 결제 없이 언제든 접근 가능)에서
  실행 환경(런타임)을 고르고 코드 저장소 주소(GIT_ADDRESS)나 설정값을 채워 완전히 새로운 서버
  종류로 공개 — 예: "Node.js 봇" 런타임 + 내 GitHub 주소 = "내 디스코드 봇"이라는 새 프리셋. 값을
  하나도 안 채우고 베이스 종류 소개만으로도 등록 가능. 공개하면 포인트 자동 지급(하루 최대 5건).
  새 서버 만들 때 "공식/커뮤니티" 탭에서 바로 골라 시작할 수 있고, 이미 만든 서버도 시작 변수
  탭의 "저장소에서 설치하기"로 원클릭 재설치 가능. 도커 이미지·설치 스크립트 자체는 손댈 수 없어
  이미 검수된 실행환경 그대로 유지되는 안전한 구조(임의 설치 스크립트 업로드는 관리자 전용 —
  아래 "관리자 Egg 가져오기" 참고). 신고 3건이 쌓이면 자동 비공개 + 지급 포인트 회수,
  `/admin/presets`에서 관리자가 직접 관리 가능. 실제 사용된 횟수로 인기순 정렬, 관리자가 만든
  것에는 인증 마크 표시
- **관리자 Egg 가져오기**: `/admin/templates`에서 Pterodactyl에 아직 없는 Egg를 JSON URL이나
  붙여넣기로 통째로 가져와 즉시 새 서버 종류로 등록(관리자 전용 — 설치 스크립트가 포함되는
  민감한 동작이라 일반 유저에게는 열지 않음). 이 세션 내내 수동으로 하던 tinker/SQL 작업을
  버튼 하나로 대체(`POST /api/admin/templates/import-egg`)
