...
채워 완전히 새로운 서버 종류("커뮤니티 Egg")를 만들어 공개하고 포인트를 받을 수도 있습니다. 가짜
...
- **홍보 포인트 & 포인트 상점**: 친구 추천(`/register?ref=내추천코드`), 디스코드 가입, 블로그/영상/
  ...
  영구히 차지하는 걸 막기 위함
- **커뮤니티 Egg**: `/dashboard/community-eggs`(대시보드 상단 탭, 결제 없이 언제든 접근 가능)에서
  실행 환경(런타임)을 고르고 코드 저장소 주소(GIT_ADDRESS)나 설정값을 채워 완전히 새로운 서버
  종류로 공개 — 예: "Node.js 봇" 런타임 + 내 GitHub 주소 = "내 디스코드 봇"이라는 새 Egg. 공개하면
  포인트 자동 지급(하루 최대 5건). 새 서버 만들 때 "공식/커뮤니티" 탭에서 바로 골라 시작할 수
  있고, 이미 만든 서버도 시작 변수 탭의 "저장소에서 설치하기"로 원클릭 재설치 가능. 도커 이미지·
  설치 스크립트 자체는 손댈 수 없어 이미 검수된 egg 그대로 유지되는 안전한 구조(임의 설치
  스크립트 업로드는 관리자 전용 — 아래 "관리자 Egg 가져오기" 참고). 신고 3건이 쌓이면 자동
  비공개 + 지급 포인트 회수, `/admin/presets`에서 관리자가 직접 관리 가능. 실제 사용된 횟수로
  인기순 정렬, 관리자가 만든 것에는 인증 마크 표시
