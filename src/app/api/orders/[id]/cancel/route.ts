 ...
 *
 * isAdminGrant가 false인(=유저가 실제로 결제했는데 아직 서버 종류만 안 고른) 주문은 여기서
 * 취소할 수 없다 — 실제로 낸 돈이 걸려있어서 이 버튼 하나로 조용히 취소되면 안 되고, 환불은
 * 이용약관에 따라 별도 절차(문의)로 처리해야 한다.
 */
export async function POST(
  ...
  if (!order.isAdminGrant) {
    return NextResponse.json(
      { error: "결제한 주문은 여기서 취소할 수 없어요. 환불이 필요하면 디스코드 /문의로 연락해주세요." },
      { status: 409 },
    );
  }
