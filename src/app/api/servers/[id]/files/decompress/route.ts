...
const schema = z.object({ directory: z.string().min(1), file: z.string().min(1) });
...
export async function POST(
  ...
  try {
    await PteroClient.decompressFile(server.pterodactylIdentifier, parsed.data.directory, parsed.data.file);
    return NextResponse.json({ ok: true });
  } catch (err) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "압축 해제 중 오류가 발생했습니다." },
      { status: 502 },
    );
  }
}
