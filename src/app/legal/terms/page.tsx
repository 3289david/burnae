  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 prose-sm">
        ...
        <Section title="제5조의2 (커뮤니티 프리셋 이용에 관한 특칙)">
          <ul className="list-disc pl-5 space-y-1">
            <li>이용자는 회사가 제공하는 검수된 실행 환경(도커 이미지·설치 스크립트) 위에 코드 저장소
              주소나 설정값을 채워 &ldquo;커뮤니티 프리셋&rdquo;을 만들어 다른 이용자와 공유할 수 있습니다.
              도커 이미지·설치 스크립트 자체는 이용자가 임의로 지정하거나 업로드할 수 없으며, 회사가
              사전에 검수한 실행 환경만 기반으로 사용됩니다.</li>
            <li>커뮤니티 프리셋에 포함된 코드 저장소 주소, 설정값, 이름·설명 등 콘텐츠는 이를 등록한
              이용자의 책임 하에 있으며, 회사는 그 내용의 정확성·안전성·적법성을 보장하지 않습니다.
              다른 이용자가 공개한 커뮤니티 프리셋을 선택해 발생하는 결과(예: 코드 오류, 원치 않는
              동작)는 이를 선택한 이용자 본인이 검토·확인할 책임이 있습니다.</li>
            <li>부적절하거나 문제가 있는 커뮤니티 프리셋은 다른 이용자의 신고를 통해 일정 건수 누적 시
              자동으로 비공개 처리되며, 등록자에게 지급됐던 포인트도 함께 회수될 수 있습니다. 회사는
              관리자 권한으로 언제든 특정 커뮤니티 프리셋을 비공개·삭제할 수 있습니다.</li>
            <li>커뮤니티 프리셋 공개 시 지급되는 포인트는 어뷰징 방지를 위해 계정당 일일 지급 한도가
              적용될 수 있습니다.</li>
          </ul>
        </Section>
