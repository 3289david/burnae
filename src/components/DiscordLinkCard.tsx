...
export default function DiscordLinkCard({ linked }: { linked: boolean }) {
  ...
  if (linked) {
    return (
      <div className="card p-5">
        ...
        <p className="text-xs text-text-dim mt-1">디스코드 봇에서 /서버목록, /상태 명령어를 쓸 수 있어요.</p>
      </div>
    );
  }
  ...
  return (
    <div className="card p-5">
      ...
      <p className="text-xs text-text-dim mt-1">봇에서 서버 상태 확인, 알림을 받으려면 연동하세요.</p>
      {code ? (
        <div className="mt-3">
          <p className="text-xs text-text-dim">디스코드 봇 채널에서 아래 명령어를 입력하세요 (10분 이내):</p>
      ...
      {INVITE_URL && (
        <a href={INVITE_URL} target="_blank" rel="noreferrer" className="block text-xs text-accent mt-3">
          내 디스코드 서버에 Burnae 봇 초대하기 →
        </a>
      )}
    </div>
  );
